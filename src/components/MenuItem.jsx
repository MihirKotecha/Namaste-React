...
const MenuItem = (parmas) => {
  console.log(parmas);
  const { name, description, imageId } = parmas.info;
  let { price } = parmas.info;
  isNaN(price) ? (price = parmas.info.variantsV2.pricingModels[0].price): price = price;
  console.log(price);

  return (
    <div className=" rounded-lg bg-white menu-item border-2 border-solid shadow-2xl  m-4 h-[250px] flex justify-end items-center w-full max-w-1/4">
